...
export function Chat({ messages, setMessages, onAnalyze }: Props) {
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between gap-2">
          ...
          <button
            ...
            onClick={onAnalyze}
            disabled={!canAnalyze}
            className="shrink-0 rounded-full border border-slate-300 px-3 py-1.5 text-xs sm:text-sm font-medium text-slate-700 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            診断結果を見る
          </button>
        </div>
